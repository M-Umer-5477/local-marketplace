...
// import AddressSelector from "@/components/AddressSelector"; // ✅ Import the Address Selector
...
//           {/* --- NEW: ADDRESS SELECTOR (Desktop Only) --- */}
...
//                       <BadgePercent className="h-5 w-5 text-primary"/> Martly
//                    </SheetTitle>
//                  </SheetHeader>
                 
//                  {/* --- NEW: ADDRESS SELECTOR (Mobile) --- */}
//                  <div className="mt-6">
//                     <AddressSelector />
//                  </div>
  ...
  return (
    <header className="sticky top-0 z-50 w-full border-b border-border/40 bg-background/80 backdrop-blur-xl supports-backdrop-filter:bg-background/60">
      <div className="container flex h-16 max-w-screen-2xl items-center justify-between px-4 md:px-8">
        ...
        {/* --- 2. DESKTOP NAVIGATION (BALANCED & DYNAMIC) --- */}
        <nav className="hidden lg:flex items-center gap-6 text-sm font-medium">
          ...
          {/* Show My Orders directly in navbar if logged in */}
          {session && (
          ...
          {/* Show Dashboard directly in navbar if they are a vendor */}
          
          
          {/* Show Become a Seller ONLY if logged out */}
          {!session && (
            <Link href="/vendor/register" className="text-muted-foreground transition-colors hover:text-primary">
              Become a Seller
            </Link>
          ...
          <div className="md:hidden flex items-center">
             <Sheet open={open} onOpenChange={setOpen}>
               ...
               <SheetContent side="right" className="flex flex-col h-full w-[85vw] sm:w-[350px] overflow-y-auto px-6 py-6">
                 ...
                 <div className="bg-muted/30 p-3 rounded-xl border mb-6">
                    <p className="text-xs font-semibold text-muted-foreground mb-2 uppercase tracking-wider">Delivering To</p>
                    <AddressSelector />
                 </div>
                 
                 <div className="flex flex-col gap-2 flex-1">
                    <Link href="/" onClick={() => setOpen(false)} className="flex items-center gap-3 px-3 py-2.5 rounded-lg hover:bg-muted transition-colors text-base font-medium">
                        <Home className="h-5 w-5 text-muted-foreground" />
                        Home
                    </Link>
                    <Link href="/shops" onClick={() => setOpen(false)} className="flex items-center gap-3 px-3 py-2.5 rounded-lg hover:bg-muted transition-colors text-base font-medium">
                        <ShoppingBag className="h-5 w-5 text-muted-foreground" />
                        Browse Shops
                    </Link>

                    {!session && (
                      <Link href="/vendor/register" onClick={() => setOpen(false)} className="flex items-center gap-3 px-3 py-2.5 rounded-lg hover:bg-muted transition-colors text-base font-medium">
                    ...
                    {session && (
                      <>
                        ...
                        <Link href="/orders" onClick={() => setOpen(false)} className="flex items-center gap-3 px-3 py-2.5 rounded-lg hover:bg-muted transition-colors text-base font-medium">
                 ...
                 <div className="border-t pt-6 mt-6 space-y-6">
                    <div className="flex items-center justify-between px-2">
                       <span className="text-sm font-medium text-muted-foreground">App Theme</span>
                       <ModeToggle />
                    </div>
                    ...
                    {session ? (
                       <div className="space-y-4">
                          ...
                          <Button className="w-full justify-center bg-red-50 hover:bg-red-100 text-red-600 border border-red-200" variant="outline" onClick={() => { handleLogout(); setOpen(false); }}>
                             <LogOut className="mr-2 h-4 w-4" /> Logout
                          </Button>
                       </div>
                    ) : (
                       <div className="grid grid-cols-2 gap-3">
                          <CheckoutAuthModal>
                              <Button variant="outline" className="w-full h-10">Login</Button>
                          </CheckoutAuthModal>
                          <CheckoutAuthModal>
