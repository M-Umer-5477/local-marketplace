...
//             // Scenario A: Cart was created by a Guest (No Owner)
//             // ACTION: Transfer ownership to this user (Keep items)
//             if (!savedOwner) {
//                 setCart(parsedCart);
//                 localStorage.setItem("cartOwner", session.user.email);
//             } 
            
//             // Scenario B: Cart belongs to a DIFFERENT user
//             // ACTION: Wipe it (Security)
//             else if (savedOwner !== session.user.email) {
//                 setCart({ shopId: null, shopName: "", items: [] });
//                 localStorage.removeItem("myAppCart");
//                 localStorage.setItem("cartOwner", session.user.email);
...
//             // Scenario C: Cart belongs to THIS user
//             // ACTION: Keep it
//             else {
//                 setCart(parsedCart);
//             }
...
//         else {
//             // Just show whatever is in local storage
//             setCart(parsedCart);
//         }
//       }
//     }
//   }, [session]); // Runs whenever session changes (e.g., after login)

//   // --- SAVE CART ON CHANGE ---
//   useEffect(() => {
//     // Only save if cart has been initialized/modified
//     localStorage.setItem("myAppCart", JSON.stringify(cart));
    
//     // If logged in, ensure ownership tag is set
//     if (session?.user?.email) {
//       localStorage.setItem("cartOwner", session.user.email);
//     }
//   }, [cart, session]);
